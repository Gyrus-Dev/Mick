...
_spinner.start("Starting PostgresAI...")
from .agent import ag_pg_manager
from google.adk.runners import Runner
from google.adk.memory import InMemoryMemoryService
from google.genai import types
from src.postgres_ai.adksession import PostgresADKSession as pgsession
from src.postgres_ai.adkstate import PostgresState as pgstate
from src.postgres_ai.adkrunner import ADKRunner
from src.postgres_ai.telemetry import tracer as _otel_tracer, shutdown as _otel_shutdown
_spinner.stop()

root_agent = ag_pg_manager
...
def _update_query_display(total_count: int) -> None:
    ...
    sys.stdout.write(f"\033]0;PostgresAI  |  Objects created: {total_count}\007")
    sys.stdout.flush()
    if total_count > 0:
...
async def call_agent_and_print(
    ...
    try:
        async for event in runner_instance.run_async(user_id=user_id, session_id=session_id, new_message=user_content):
            ...
            if state:
                if os.environ.get("POSTGRESAI_DEBUG", "").lower() == "true":
...
async def main(message, runner, pg_session, memory_bank_service, service, chat_history: list | None = None, query_offset: int = 0):
    ...
    final_response, queries_executed = await call_agent_and_print(
        ...
        pg_session.id,
        enriched_message,
        initial_state=pg_session.state,
...
def _write_session_queries(all_queries: list[str]) -> None:
    ...
    with open(filepath, "w") as f:
        f.write(f"-- PostgresAI session queries — {timestamp}\n\n")
...
async def interactive():
    ...
    banner.append("╚═╝     ╚═╝   ╚═╝    ╚═════╝   ╚═╝  ╚═╝\n", style="bold cyan")
    banner.append("\n")
    banner.append("                                             ╰─ by Gyrus Inc · PostgreSQL AI Assistant ─╯\n", style="dim")
    _console.print(Panel(banner, subtitle="[dim]Your PostgreSQL AI Assistant  ·  type [bold]exit[/bold] to quit[/dim]", border_style="cyan"))
    ...
    pg_state.init_postgres_state()

    pg_session = pgsession(user_id=os.environ["APP_USER_ID"], app_name=os.environ["APP_NAME"], state=pg_state)
    logger.debug("interactive: session created id=%s", pg_session.id)
    _console.print(f"[dim]Session:[/dim] [bold]{pg_session.id}[/bold]")

    _adk_session, service = await pg_session.create_session()
    ...
    _intro = (
        "👋  **Hi! I'm PostgresAI** — your PostgreSQL AI assistant.\n\n"
        "Here's what I can do for you:\n\n"
        "- 🏗  **Build from scratch** — describe what you need and I'll design and create "
        "the full PostgreSQL infrastructure: databases, schemas, tables, indexes, views, "
        "functions, triggers, roles, policies, and more.\n"
        "- 🔍  **Understand your existing setup** — I can inspect your live PostgreSQL "
        "environment, map your architecture, and answer questions about what's already there.\n"
        "- 📊  **Monitor performance** — I can query pg_stat_* views to show you slow queries, "
        "connection counts, table bloat, and more.\n\n"
        "**Getting started tip:**\n"
        ...
        "> *\"Inspect the public schema and give me an overview of its tables and indexes.\"*\n\n"
    ...
    _console.print(Panel(
        Markdown(_intro),
        title="[bold cyan]PostgresAI[/bold cyan]",
        border_style="cyan",
        padding=(1, 2),
    ))
    ...
    while True:
        ...
        with _otel_tracer.start_as_current_span("postgresai.user_request") as _span:
            _span.set_attribute("query.length", len(user_input))
            response, queries = await main(user_input, runner, pg_session, memory_bank_service, service, chat_history, query_offset=len(session_queries))
        ...
        _console.print(Panel(Markdown(full_response), title="[bold blue]PostgresAI[/bold blue]", border_style="blue", padding=(1, 2)))
        if queries:
