...
class PostgresState(ADKState):
    def __init__(self, user_name, pg_user, password, host, port=5432, database=None):
        super().__init__()
    ...
    def init_postgres_state(self):
        ...
        self.add_user_state("PG_USER", self.pg_user)
        self.add_user_state("PG_PASSWORD", self.password)
        self.add_user_state("QUERIES_EXECUTED", [])
        self.add_app_state("PG_HOST", self.host)
        self.add_app_state("PG_PORT", self.port)
        self.add_app_state("PG_DATABASE", self.database)
